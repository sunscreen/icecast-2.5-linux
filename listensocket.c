...
#ifdef USE_POLL
static listensocket_t * listensocket_container_accept__inner_poll(listensocket_container_t *self, int timeout)
{
...
connection_t * listensocket_container_accept(listensocket_container_t *self, int timeout)
{
    ...
    thread_mutex_lock(&self->lock);
#ifdef USE_POLL
    ls = listensocket_container_accept__inner_poll(self, timeout);
#endif
#ifdef USE_EPOLL
...
//    p->events = EPOLLIN | EPOLLET;
    ...
    if (res != 0 && errno == EEXIST) {
        res = epoll_ctl(epollinstance, EPOLL_CTL_MOD,self->sock, p);
    }

    if(res == EBADF)
    {
        fprintf(stderr, "Failed to add file descriptor to epoll in epoll_fill\n");
    }
